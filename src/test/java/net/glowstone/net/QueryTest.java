...
public class QueryTest {
     ...
     */
    private static final byte[] HANDSHAKE_RECV = DatatypeConverter.parseHexBinary("FEFD0900000001");
    private static final byte[] HANDSHAKE_SEND = DatatypeConverter.parseHexBinary("09000000013935313333303700");
    private static final byte[] BASIC_STATS_RECV = DatatypeConverter.parseHexBinary("FEFD00000000010091295B");
    private static final byte[] BASIC_STATS_SEND = DatatypeConverter.parseHexBinary("000000000141204D696E6563726166742053657276657200534D5000776F726C64003200323000DD633132372E302E302E3100");
    private static final byte[] FULL_STATS_RECV = DatatypeConverter.parseHexBinary("FEFD00000000010091295B00000000");
    private static final byte[] FULL_STATS_SEND = DatatypeConverter.parseHexBinary("000000000173706C69746E756D008000686F73746E616D65004120"
    ...
    public void setup() throws Exception {
        ...
        server = new QueryServer(glowServer);
        random = mock(Random.class);
        installRandomMock(server, random);
    ...
    private void installRandomMock(QueryServer server, Random mock) throws Exception {
        Field field = QueryServer.class.getDeclaredField("random");
        field.setAccessible(true);
        field.set(server, mock);
    }

    @Test
    public void testHandshake() throws Exception {
        QueryHandler handler = new QueryHandler(server);
    ...
    public void testFullStats() throws Exception {
        ...
        when(glowServer.getMotd()).thenReturn("A Minecraft Server");
        when(glowServer.getOnlinePlayers()).thenReturn(Arrays.asList(p1, p2));
        when(glowServer.getMaxPlayers()).thenReturn(20);
        when(glowServer.getPort()).thenReturn(25565);
        when(glowServer.getWorlds()).thenReturn(Arrays.asList(world));
        when(glowServer.getIp()).thenReturn("");
